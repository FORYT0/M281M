...
.signal-box { border-radius:10px; padding:1.2rem 1.5rem; text-align:center; border:2px solid; margin-bottom:.5rem; }
...
.trade-table th { background:#161b22; color:#8b949e; padding:.55rem .8rem;
    text-align:left; border-bottom:1px solid #30363d; font-weight:500;
...
.ks-alert { background:#2e0d0d; border:1px solid #f85149; border-left:4px solid #f85149;
    border-radius:8px; padding:1rem 1.2rem; margin:1rem 0; color:#ffa198;
    font-size:.88rem; font-weight:600; }
</style>
""", unsafe_allow_html=True)

# ── Base layout — NO margin key ─────────────────────────────────────────────────
CHART_LAYOUT = dict(
...
def calc_metrics(eq, tr):
    ...
    if not eq.empty:
        ...
        m["pos_value"] = float(eq.get("position_value", pd.Series([0])).iloc[-1])
        eq2 = eq.copy()
        eq2["peak"] = eq2["total_value"].cummax()
        eq2["dd"]   = (eq2["total_value"] - eq2["peak"]) / eq2["peak"] * 100
        m["dd"]     = float(eq2["dd"].min())
        m["cur_dd"] = float(eq2["dd"].iloc[-1])
...
def chart_pnl_dist(tr):
    ...
    bar_colors = ["#3fb950" if v >= 0 else "#f85149" for v in done["pnl"]]
    fig = go.Figure()
    fig.add_trace(go.Histogram(x=done["pnl"], nbinsx=30,
        marker_color=bar_colors, marker_line_color="#0d1117",
        marker_line_width=1, name="PnL"))
    mean = done["pnl"].mean()
    fig.add_vline(x=mean, line_dash="dash", line_color="#d29922",
        annotation_text=f"Mean: ${mean:.2f}", annotation_font_color="#d29922")
    fig.update_layout(**CHART_LAYOUT, title="Trade PnL Distribution",
...
def chart_cumulative_pnl(tr):
    ...
    dot_colors = ["#3fb950" if v >= 0 else "#f85149" for v in done["cum_pnl"]]
    fig = go.Figure()
    ...
    fig.update_layout(**CHART_LAYOUT, title="Cumulative PnL",
        height=300, margin=dict(l=50, r=20, t=40, b=40))
    return fig
...
def chart_agent_weights(data):
    aw = data.get("agent_weights") or data.get("model_performance", {})
    ...
    fig.update_layout(**CHART_LAYOUT, title="Agent Performance / Weights",
        ...
        yaxis=dict(gridcolor="#21262d", range=[0, max(vals)*1.2+5] if vals else [0,100]))
    return fig
...
with st.sidebar:
    ...
    if status:
        ...
        if ks.get("is_killed"):
            st.markdown('<span class="badge kill">🚨 KILL SWITCH</span>', unsafe_allow_html=True)
        ...
        st.markdown(f'<div class="signal-box {sig_cls}"><div class="dir">{sig}</div><div class="conf">Confidence: {conf:.1%}</div></div>', unsafe_allow_html=True)
        if "price" in status:
...
<div class="top-banner">
  ...
  <div class="meta">
    <span style="font-size:.85rem;color:{ret_color};font-weight:700;">{ret_sign}{m['ret_pct']:.2f}% return</span>
...
ks = status.get("kill_switch", {})
if ks.get("is_killed"):
    st.markdown(f'<div class="ks-alert">🚨 KILL SWITCH — {ks.get("kill_reason","Unknown")} — Trading halted.</div>', unsafe_allow_html=True)
...
<div class="kpi-grid">
  ...
  <div class="kpi red">
    ...
    <div class="delta neu">{m['cur_dd']:.2f}% current</div>
  </div>
</div>
""", unsafe_allow_html=True)
...
with tab1:
    ...
    st.plotly_chart(f, use_container_width=True) if f else st.info("Equity chart appears once trading starts.")
    c1, c2 = st.columns(2)
    with c1:
...
with tab2:
    ...
    if not tr_df.empty:
        ...
        for _, row in show.iterrows():
            ...
            pnl_str  = (f'<span class="{"win" if pnl_val>0 else "loss"}">${pnl_val:+.2f}</span>'
                        if pd.notna(pnl_val) else "—")
            action   = str(row.get("action", row.get("side","—")))
            price    = f"${row['price']:,.2f}" if "price" in row and pd.notna(row.get("price")) else "—"
            amount   = (f"{row['amount']:.5f}" if "amount" in row and pd.notna(row.get("amount"))
                        else (f"{row.get('size',''):.5f}" if "size" in row else "—"))
        ...
        <table class="trade-table">
          <thead><tr><th>Timestamp</th><th>Action</th><th>Price</th><th>Amount</th>
            <th>PnL</th><th>Confidence</th><th>Reason</th></tr></thead>
          <tbody>{rows_html}</tbody>
        </table>""", unsafe_allow_html=True)
    else:
        st.info("No trades yet — system is running, waiting for signals.")
...
with tab3:
    ...
    with c2:
        ...
        perf_data = {
            "Metric": ["Total Return","Total PnL","Win Rate","Profit Factor",
                       "Sharpe Ratio","Avg Win","Avg Loss","Max Drawdown"],
...
with tab4:
    ...
    st.plotly_chart(f, use_container_width=True) if f else st.info("Agent chart — awaiting live session data")
    ...
    if ks:
        ...
        st.info("Kill switch status appears once trading session starts.")
    if summary:
        with st.expander("📋 Full Session Summary JSON"):
            st.json(summary)

# ── Footer ─────────────────────────────────────────────────────────────────────
st.markdown(f"<div style='text-align:center;color:#30363d;font-size:.75rem;margin-top:2rem;'>M281M Trading Dashboard · {now_str}</div>", unsafe_allow_html=True)
